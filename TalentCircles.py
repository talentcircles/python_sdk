...
class TalentCircles:
	...
	def get_posessions(self, resource, resource_id, possession, additional_params=None):
		if not resource_id:
			return
		...
		if additional_params is None:
			...
			response = requests.request("GET", url, auth=HTTPDigestAuth(self.app_id, self.api_key), params=additonal_params, verify=self.verify_ssl)
	...
	def create_resource(self, resource, resource_data, default_fields=None):
		...
		if default_fields is None:
			...
			payload = default_fields.copy()   # start with resource_data's keys and values
			payload.update(resource_data)
		response = requests.request("POST", url, data=payload, auth=HTTPDigestAuth(self.app_id, self.api_key), verify=self.verify_ssl)
		return json.loads(response.text)

	def create_possession(self, resource, resource_id, possession, possession_data, default_fields=None):
		url = self.url + resource + '/' + str(resource_id) + '/' + possession
		if default_fields is None:
			payload = possession_data
		else:
			payload = default_fields.copy()   # start with possession_data's keys and values
			payload.update(possession_data)
		response = requests.request("POST", url, data=payload, auth=HTTPDigestAuth(self.app_id, self.api_key), verify=self.verify_ssl)
		return json.loads(response.text)

	def search_resource(self, resource, search_params, page=1, results_per_page=10):
		querystring = {
			"page":page,
		...
		response = requests.request("GET", url, auth=HTTPDigestAuth(self.app_id, self.api_key), params=querystring, verify=self.verify_ssl)
		return json.loads(response.text)
	...
	def update_resource(self, resource, resource_id, resource_data):
		...
		if resource_id:
			if type(resource_id) is list:
				...
				url += "/" + str(resource_id)
		response = requests.request("PUT", url, data=resource_data, auth=HTTPDigestAuth(self.app_id, self.api_key), verify=self.verify_ssl)
		return json.loads(response.text)
	...
	def search_jobs(self, search_params, page=1, results_per_page=10):
		result = self.search_resource('jobs', search_params, page=page, results_per_page=results_per_page)
		return result['jobs']
	...
	def get_user_stories(self, user_id, additonal_params=None):
		...
		if additonal_params is None:	
			...
			result = self.get_posessions('users', user_id, 'stories', addtional_params)
	...
	def search_users(self, search_params, page=1, results_per_page=10):
		result = self.search_resource('users', search_params, page=page, results_per_page=results_per_page)
		return result['users']
	...
	def validate_user_data(self, user_data):
		...
		if 'zipcode' not in validated.keys() and 'city' not in validated.keys() and 'state' not in validated.keys():
			raise ValueError, 'Missing location data for new user. Please include either a zipcode, or a city and state.'
		return validated
	...
	def get_circle_jobs(self, circle_id, additonal_params=None):
		...
		if additonal_params is None:
			...
			result = self.get_posessions('circles', circle_id, 'jobs', additonal_params)
		return result['jobs']
	...
	def post_circle_story(self, circle_id, story_data):
		...
		result = self.create_possession('circles', circle_id, 'stories', valid_data)
		return result['story']
